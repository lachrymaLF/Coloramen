...
const out_path = (() => {
    ...
    switch (process.platform) {
        case 'win32':
            try {
                fs.closeSync(fs.openSync(path.join(app_path, "template.ffx"), 'a'));
                return try_path.replace(new RegExp('\\\\', 'g'), '\\\\');
            } catch (err) {
                ...
                return fallback.replace(new RegExp('\\\\', 'g'), '\\\\');
            }
        case 'darwin':
            try {
                fs.accessSync(try_path, fs.constants.W_OK);
                return try_path;
            } catch (err) {
                const fallback = path.join(process.env.HOME, "Library", "Application Support", "Adobe", "CEP", "extensions", "com.lachrymalf.coloramen",  "coloramen.ffx");
